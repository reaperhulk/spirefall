...
import { DOCTRINES, doctrineDescription, type DoctrineId } from '../data/doctrines'
...
export function BuildDoctrine({ state, choose }: { state: RunState; choose: (id: DoctrineId) => void }) {
  if (state.doctrine) {
    ...
    return <section className="doctrine-summary"><strong>{DOCTRINES[state.doctrine].name}</strong> · {doctrineDescription(state.doctrine, state.rulesVersion ?? 4)}
  ...
  return <section className="doctrine-panel" aria-label="Choose a build doctrine">
    ...
    <div className="doctrine-choices">{(Object.entries(DOCTRINES) as [DoctrineId, typeof DOCTRINES[DoctrineId]][]).map(([id, d]) =>
      <button key={id} className="ghost-btn" data-testid={`doctrine-${id}`} onClick={() => choose(id)}><b>{d.name}</b><span>{doctrineDescription(id, state.rulesVersion ?? 4)}</span><small>{d.towers.filter(t => state.towers.some(owned => owned.type === t)).length} matching tower types built</small></button>)}</div>
  </section>
}
