...
// ---------------------------------------------------------------------------
// Approach lane: a narrow strip OUTSIDE the playfield wall on the spawn side
// (all six maps spawn on the left edge). During a wave, pending spawns march
// through it toward the gate at their real speed — you see the next pack
// coming before it can be attacked. In the build phase it shows the next
// wave's vanguard dimly forming up. A separate canvas, so the playfield's
// coordinate space (input, tests, the loupe) is untouched; being outside the
// board's border makes the border itself read as the wall they breach.

export const APPROACH_CELLS = 2
export const APPROACH_W = APPROACH_CELLS * CELL_PX

interface LaneGhost {
  type: string
  x: number // lane-local logical px (0 = far edge, APPROACH_W = the wall)
  seed: number
}

let lanePreviewKey = ''
let lanePreviewGhosts: LaneGhost[] = []

// Build-phase vanguard: expand the scouting report's counts into a short
// queue, mirroring the chip order (boss first, then biggest packs).
function buildPhaseGhosts(state: RunState): LaneGhost[] {
  const key = `${state.seed}|${state.wave}|${state.mapId}|${state.phase}`
  if (key === lanePreviewKey) return lanePreviewGhosts
  const ghosts: LaneGhost[] = []
  const preview = previewNextWave(state)
  if (preview) {
    const order = (Object.entries(preview.counts) as [string, number][]).sort(([ta, a], [tb, b]) =>
      ta.startsWith('boss') ? -1 : tb.startsWith('boss') ? 1 : b - a || ta.localeCompare(tb),
    )
    const units: string[] = []
    for (const [type, n] of order) {
      for (let i = 0; i < Math.min(n, 4) && units.length < 9; i++) units.push(type)
    }
    units.forEach((type, i) => {
      ghosts.push({ type, x: APPROACH_W - 10 - i * ((APPROACH_W - 18) / Math.max(1, units.length - 1)), seed: i })
    })
  }
  lanePreviewKey = key
  lanePreviewGhosts = ghosts
  return ghosts
}

function drawLaneGhost(
  ctx: CanvasRenderingContext2D,
  ghost: LaneGhost,
  gateY: number,
  alpha: number,
  ...
  const def = ENEMIES[ghost.type as keyof typeof ENEMIES]
  if (!def) return
  const jitter = (hash01(ghost.seed, 7) - 0.5) * 10
  const bob = Math.sin(nowMs / 160 + ghost.seed * 1.7) * 1.2
  const flying = !!def.flying
  const y = gateY + jitter + bob + (flying ? -7 : 0)
  const r = ghost.type.startsWith('boss') ? 10 : def.elite ? 7.5 : 5
  ctx.globalAlpha = alpha * 0.35
  ctx.fillStyle = '#000'
  ctx.beginPath()
  ctx.ellipse(ghost.x, gateY + jitter + (flying ? 6 : r * 0.8) + 2, r * 0.9, r * 0.35, 0, 0, Math.PI * 2)
  ctx.fill()
  ctx.globalAlpha = alpha
  ctx.fillStyle = enemyColor(ghost.type)
  ctx.beginPath()
  ctx.arc(ghost.x, y, r, 0, Math.PI * 2)
  ctx.fill()
  ctx.globalAlpha = 1
}

export function drawApproachLane(ctx: CanvasRenderingContext2D, session: GameSession, nowMs: number): void {
  const state = session.state
  const map = getMap(state.mapId)
  const h = MAP_HEIGHT * CELL_PX
  const gateY = (map.spawn.cy + 0.5) * CELL_PX

  // Outside the walls: darker than the battlefield, sparse speckle.
  ctx.fillStyle = '#07090c'
  ctx.fillRect(0, 0, APPROACH_W, h)
  ctx.fillStyle = 'rgba(255, 255, 255, 0.03)'
  for (let i = 0; i < 26; i++) {
    const sx = hash01(i, 11) * APPROACH_W
    const sy = hash01(i, 23) * h
    ctx.fillRect(sx, sy, 1.5, 1.5)
  }

  // The dirt road they arrive on, running off the edge of the world.
  ctx.fillStyle = 'rgba(0, 0, 0, 0.45)'
  ctx.fillRect(0, gateY - CELL_PX * 0.5, APPROACH_W, CELL_PX)
  ctx.fillStyle = 'rgba(122, 106, 82, 0.1)'
  ctx.fillRect(0, gateY - CELL_PX * 0.38, APPROACH_W, CELL_PX * 0.76)
  // Direction chevrons, pointing at the gate.
  ctx.strokeStyle = 'rgba(187, 154, 247, 0.22)'
  ctx.lineWidth = 2
  for (let i = 0; i < 2; i++) {
    const cx = APPROACH_W * (0.3 + i * 0.4)
    ctx.beginPath()
    ctx.moveTo(cx - 4, gateY - 6)
    ctx.lineTo(cx + 4, gateY)
    ctx.lineTo(cx - 4, gateY + 6)
    ctx.stroke()
  }

  if (state.phase === 'wave') {
    // Real spawn queue: each pending unit is dist = speed × time-to-spawn
    // from the gate. Slow heavies telegraph earlier; runners burst in late.
    for (const sp of state.pendingSpawns) {
      const def = ENEMIES[sp.type]
      const ticksUntil = sp.tick - state.tick
      if (ticksUntil < 0) continue
      const distPx = ((def.speed * ticksUntil) / 1000) * CELL_PX
      const x = APPROACH_W + CELL_PX * 0.5 - distPx
      if (x < -8) continue
      const edgeFade = Math.max(0, Math.min(1, (x + 8) / 30))
      drawLaneGhost(ctx, { type: sp.type, x: Math.min(x, APPROACH_W - 2), seed: sp.tick }, gateY, 0.55 * edgeFade, nowMs)
    }
  } else if (state.phase === 'build') {
    for (const ghost of buildPhaseGhosts(state)) drawLaneGhost(ctx, ghost, gateY, 0.3, nowMs)
  }

  // The wall's outer face: a thin lit edge where the lane meets the board.
  const wall = ctx.createLinearGradient(APPROACH_W - 10, 0, APPROACH_W, 0)
  wall.addColorStop(0, 'rgba(0, 0, 0, 0)')
  wall.addColorStop(1, 'rgba(122, 162, 247, 0.1)')
  ctx.fillStyle = wall
  ctx.fillRect(APPROACH_W - 10, 0, 10, h)
}
