...
import { KEYSTONE_GLASSFORGE_DAMAGE_PCT } from '../src/data/metaTree'
...
const runs=[]
for (const finding of findings) for (const seed of ['gamma',finding.biome === 'verdant' ? 'theta' : 'delta']) {
  for (const variant of ['bonus35','bonus30','bonus25','bonus20','bonus0','hp50','hp55','no_beam','no_execute','no_bounty']) {
    ...
    initial.mods.damagePct+=bonus-KEYSTONE_GLASSFORGE_DAMAGE_PCT
